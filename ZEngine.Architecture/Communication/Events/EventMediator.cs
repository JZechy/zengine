...
public class EventMediator : IEventMediator
{
    ...
    public void Subscribe<TMessage>(Action<TMessage> receiver) where TMessage : IEventMessage
    {
        ...
        if (!_receivers.ContainsKey(messageType)) _receivers.TryAdd(messageType, new ReceiverCollection<TMessage>(_loggerFactory.CreateLogger<ReceiverCollection<TMessage>>()));
    ...
    {
        Type messageType = typeof(TMessage);
        if (!_receivers.TryGetValue(messageType, out IReceiverCollection? collection)) return;

        collection.Remove(receiver);
        if (collection.Count == 0) _receivers.TryRemove(messageType, out IReceiverCollection? _);
    }
