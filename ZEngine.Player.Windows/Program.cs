...
internal static class Program
{
    ...
    {
        InputManager.Instance.RegisterKeyboardInput(new KeyboardInputPath(Key.A), context => { Console.WriteLine($"Key A: {context.Context.State}"); });

        InputManager.Instance.RegisterPointerPositionInput(context => { Console.WriteLine($"Pointer position: {context.Context.Position}"); });
    }
}