...
public abstract class GenericCallbackManager<TContext> : IDeviceCallbackManager
{
    ...
    private readonly ILogger<IDeviceCallbackManager> _logger;
    ...
    {
        if (!_callbacks.ContainsKey(path)) _callbacks.TryAdd(path, new HashSet<Action<InputContext<TContext>>>());
    ...
    {
        if (!_callbacks.ContainsKey(path)) return;
